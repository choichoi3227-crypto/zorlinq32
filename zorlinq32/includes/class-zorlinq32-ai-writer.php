    ...
    public function render_settings_page() {
        ...
        <div class="wrap zorlinq32-wrap">
            ...
            <div class="zorlinq32-settings-section">
                <h2><span class="dashicons dashicons-google"></span> Google Flow 이미지 생성</h2>
                <p class="zorlinq32-help-text">이미지 생성 시 Google Flow 창을 엽니다. 프롬프트는 자동으로 클립보드에 복사됩니다.</p>
                <p class="description">Google Flow는 외부 사이트이므로 플러그인은 로그인 세션이나 생성 화면을 제어할 수 없습니다. Flow에서 프롬프트를 붙여넣어 생성한 뒤 결과를 업로드하세요.</p>
            </div>
            <div class="zorlinq32-settings-section">
                <h2>AI 글쓰기 설정</h2>
        ...
        <script>jQuery(function($){var ajaxUrl='<?php echo esc_js( admin_url( 'admin-ajax.php' ) ); ?>',nonce='<?php echo esc_js( $nonce ); ?>'; $('#zorlinq32-ai-key-register').on('click',function(){$.post(ajaxUrl,{action:'zorlinq32_ai_add_gemini_key',nonce:nonce,api_key:$('#zorlinq32-ai-key-input').val()},function(r){if(r.success)location.reload();else $('#zorlinq32-ai-key-msg').text(r.data.message);});}); $(document).on('click','.zorlinq32-ai-key-delete',function(){$.post(ajaxUrl,{action:'zorlinq32_ai_delete_gemini_key',nonce:nonce,index:$(this).data('index')},function(){location.reload();});}); $('#zorlinq32-ai-worker-save').on('click',function(){$.post(ajaxUrl,{action:'zorlinq32_ai_save_worker_urls',nonce:nonce,search_worker_url:$('#zorlinq32-ai-search-worker-url').val(),search_worker_secret:$('#zorlinq32-ai-search-worker-secret').val()},function(r){$('#zorlinq32-ai-worker-msg').text(r.success?'저장되었습니다.':r.data.message);});});});</script>
        <?php
    }
...
오직 확장된 3~4개 문장만 출력하세요:";
    ...
    public function ajax_generate_image_prompt() {
        ...
        /* Google Flow receives this prompt through the user-controlled popup handoff. */
...
SVG;
    ...
    /** Open a user-controlled Google Flow popup. */
    public function ajax_google_flow_generate() {
        check_ajax_referer( 'zorlinq32_ai_nonce', 'nonce' );
        $prompt = isset( $_POST['prompt'] ) ? sanitize_textarea_field( wp_unslash( $_POST['prompt'] ) ) : '';
        if ( '' === $prompt ) wp_send_json_error( [ 'message' => 'Google Flow에 보낼 프롬프트가 없습니다.' ] );
        wp_send_json_success( [
            'provider' => 'google_flow',
            'flow_url' => 'https://labs.google/fx/tools/flow',
            'prompt' => $prompt,
            'message' => 'Google Flow 창이 열렸습니다. 프롬프트는 클립보드에 복사되었습니다.',
        ] );
    }
